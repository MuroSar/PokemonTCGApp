...
import com.globant.data.mapper.PokemonSupertypeRoomMapper
...
class PokemonSupertypeDatabaseImpl(private val pokemonDao: PokemonDao) : PokemonSupertypeDatabase {
    ...
    private val pokemonSupertypeRoomMapper = PokemonSupertypeRoomMapper()
    ...
    override fun getLocalPokemonSupertypes(): Result<List<PokemonSupertype>> {
        ...
        pokemonSupertypes.let {
            if (it.isNotEmpty())
                return Result.Success(pokemonSupertypeRoomMapper.transform(pokemonDao.getPokemonSupertypes()))
        }
    ...
    override fun insertLocalPokemonSupertypes(pokemonSupertypes: List<PokemonSupertype>) {
        pokemonSupertypes.forEach { pokemonDao.insertPokemonSupertype(pokemonSupertypeRoomMapper.transformToPokemonSupertypeRoom(it)) }
    }
}
